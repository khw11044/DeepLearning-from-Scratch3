...
class Variable:
    __array_priority__ = 200    # '연산자 우선순위' 지정 해야한다. Variable 인스턴스 속성에 __array_priority__ 를 추가하고 그 값을 큰 정수로 설정해야한다.
# 좌항인 ndarray 인스턴스의 __add__ 메서드가 호출된다.
# 하지만 우리는 우항인 Variable 인스턴스의 __radd__ 메서드가 호출되길 원한다.

    def __init__(self, data, name=None):
        if data is not None:
            if not isinstance(data, np.ndarray):                            # 입력받는 데이터가 ndarray 구조가 아니면 오류 발생
                raise TypeError('{} is not supported, 지원안함 core.py확인'.format(type(data)))
    ...
    def backward(self, retain_grad=False, create_graph=False):  
        # retain_grad=True : 모든 변수가 미분결과(기울기)를 유지, False면 중간 변수의 미분값을 모두 None으로 재설정
